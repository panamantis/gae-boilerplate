...
class SubscribeHandler(BaseHandler):
    ...
    def get(self):
        """ Returns a simple HTML for subscription info and iframe """
	...
	exp_time = curr_time + 3600
        ...
        # common JWT 
        jwt_info = {'iss': self.app.config.get('SELLER_ID'),
                    'aud': 'Google',
        ...
        token_1 = jwt.encode(jwt_info, self.app.config.get('SELLER_SECRET'))
        ...
        request_info.update({'name': 'Deluxe Posters - Monthly Subscription', 
                             ...
                             'initialPayment': initial_payment_info,
                             'recurrence': recurrence_info})
        token_2 = jwt.encode(jwt_info, self.app.config.get('SELLER_SECRET'))
    ...
    def post(self):
        ...
        if encoded_jwt is not None:
            ...
            decoded_jwt = jwt.decode(str(encoded_jwt), self.app.config.get('SELLER_SECRET'))

            # validate the payment request and respond back to Google
            if decoded_jwt['iss'] == 'Google' and decoded_jwt['aud'] == self.app.config.get('SELLER_ID'):
        ...
        return self.redirect_to('subscribe')
